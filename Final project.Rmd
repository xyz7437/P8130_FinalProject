...
cv_df = 
  ...
  mutate(
    mod_1 = map(train, ~lm(crime_rate ~ pop18 + bagrad + poverty + 
                    pcincome + region + pcarea + pcbeds, data = .x)),
    mod_2 = map(train, ~lm(crime_rate ~ pop18 + bagrad + pcincome + region + pcarea + pcbeds + poverty + pcincome*poverty + pcincome*bagrad + pop18*bagrad, data = .x))) %>% 
  mutate(
